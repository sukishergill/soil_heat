...
Grid.th = pi/4;      Grid.dth = pi/64;       Grid.Nth = Grid.th/Grid.dth; 

r = linspace(Grid.dr, Grid.r, Grid.Nr);
th = linspace(0, Grid.th, Grid.Nth);
...
P_w = (9.8*(Fluid.rho_w/1000)+ 1.01*10^5)*...
    (ones(Grid.Nth,Grid.Nr));                       % water pressure


% initial values
% initial temperature (Kelvin)
T = (10+273.15)*ones(Grid.Nth,Grid.Nr);     

% sink term that represents the heat comsumed by co-boiling
Q = zeros(Grid.Nth, Grid.Nr);    

% Initial saturations
S_g = zeros(Grid.Nth,Grid.Nr);           % initial gas saturation
S_n = 0.01*ones(Grid.Nth,Grid.Nr);       % initial water saturation
S_w = ones(Grid.Nth,Grid.Nr) - S_n;      % initial NAPL saturation
Fluid.S_r = 0.13;                             % residual wetting saturation
Fluid.S_gcr = 0.15;                           % critical gas saturation
...
while t < 5000
   ...
   % compute temp
   T = temp_rad_2D(Grid, T, Q, lambda, heat_cap, f_i, f_o);
   T = abs(T);
   t = t + Grid.dt;
